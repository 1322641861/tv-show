<template>
	<view class="whole-page">
		...
		<!--prizeType  1-蜜糖 2-优先抢购卡 3-盲盒 4-藏品 5-第三方平台会员 6-实物 7-红包', -->
		<!-- prizeState;    // 奖品状态，1：待领取，2：已领取/待发货，3：已发货 -->
		  <view class="gift-list" v-for="(item,index) in giftList">
				...
				<view class="gift-conet">
					  ...
					  <view class="gift-bottom">
							...
							<view class="gift-right" v-if="item.prizeState == 2" @click="goprizeRecord(item)">
								  {{item.prizeType == 6 ? ' 待发货':'已领取'}}  
								  <image src="../../../static/images/you.png"></image>
							</view>
						    <view class="gift-text" v-if="item.prizeState == 3 && item.prizeType == 6"
						   ...
						   <view class="gift-botton" v-if="item.prizeState == 1" @click="gorece(item)">领取</view>
					  </view>
				</view>
		  </view>
	</view>
...
<script>
	export default {
		...
		methods: {
			...
			gorece(data){
			  uni.navigateTo({
			     url: '../home/receiveRrize?data='+ JSON.stringify(data)
			  });
			},
		  getgoodslist(){
		     let that = this
