<template>
	<view class="whole-page">
		 <view class="bottom-img" v-if="Detail_info.oneCategoryName == '3D'">
		  		<image :src="img_url+Detail_info.lbPics" style="width:100%;" mode="widthFix"></image>
		  </view>
		  <view class="bottom-img" v-if="Detail_info.oneCategoryName == '图片'">
			    <image :src="img_url+Detail_info.lbPics" style="width:100%;" mode="widthFix"></image>
		  </view> 
		  <view class="whole-video" v-if="Detail_info.oneCategoryName == '视频'">
			    <video :src="img_url+Detail_info.contentUrl" :autoplay="true" :loop="true" :controls="false"
				       object-fit="cover"></video>
		  </view>
		  <view class="whole-audio" v-if="Detail_info.oneCategoryName == '音乐'">
		  	    <image :src="img_url+Detail_info.creatorAvatar" style="filter:blur(80rpx)"></image>
				<view class="audio-post">
					<view class="audio-conet">
						<!-- :class="is_play == 'play' ? 'funs':''" -->
						  <view class="audio-img funs">
								<image src="../../../static/images/audio2.png"></image>
								<view class="post-conet">
									  <view class="conet-img">
										  <image :src="img_url+Detail_info.creatorAvatar"></image>
									  </view>
								</view>
						  </view>
					</view>
					...
					<view class="page-section">
						  <audio  @play="goplay" @pause="gopause" 
							   :src="img_url+Detail_info.contentUrl" 
							   :poster="img_url+Detail_info.creatorAvatar"   controls></audio>
					</view>
					<!-- loop -->
				</view>		
		  </view>
		  <view class="back-padd">
		        <view class="box-name"> {{Detail_info.itemName}}</view>
			    <view class="label-whole">
					<view class="box-num">
						  <view class="left-num">限量</view>
						   <view class="right-num"> {{Detail_info.stockTotal}}份</view>
					</view>
					 <view class="label-item" v-for="item in Detail_info.rarenessNames">
						   {{item}}
					 </view>	
			    </view>
			    <view class="box-info">
					  ...
					  <view class="info-name">{{Detail_info.creatorName}}</view>
					 <view class="info-text">创作者</view>
				</view>
				<view class="whole-tiem" v-if="Detail_info.saleType == 4">
					  <view class="item-top">
							 <view class="left-item">预约流程</view>
							 <view class="right-item">已预约{{Detail_info.reservePeople}}人</view>
					  </view>
					   <view class="spot-top">
							 <view class="top-item" style="text-align: left;">预约抽签</view>
							 <view class="top-item" style="padding-right: 30upx;">预约结束</view>
							 <view class="top-item" style="padding-left: 30upx;">中签付款</view>
					   ...
					   <view class="spot-top">
							 <view class="item-button" style="text-align: left;">{{Detail_info.drawLotsStartTime}}</view>
							 <view class="item-button">{{Detail_info.drawLotsEndTime}}</view>
							 <view class="item-button">{{Detail_info.priorityBuyTime}}</view>
							 <view class="item-button" style="text-align: right;">{{Detail_info.publicBuyTime}}</view>
					   </view>
				</view>
				<view class="Friends-whole" v-if="Detail_info.buyStatus == 7 && Detail_info.isParticipate == 1">
					   <view class="Friends-title">邀请好友增加中签机会</view>
						<view class="Friends-coent">
							 <view class="Friends-left">
								   <view class="Friends-img" v-for="(item,index) in Detail_info.inviteUsers"
										 v-if="index < 11">
							 ...
							 <view class="Friends-right">中签率X{{Detail_info.inviteUsers.length}}</view>
						</view>
				 </view>
				 <view class="Friends-whole" v-if="Detail_info.isParticipate == 1">
					   <view>
					   <view class="Friends-title" style="float:left;">
								已获得抽签号码（{{Detail_info.lotteryCodes.length}}个）</view>
						   <view class="right-chat" @click="gocode('bottom')">
								查看所有号码<image src="../../../static/images/you2.png"></image>
						   </view>
					   </view>
					   <view class="whole-code">
							 <view class="codeitem" v-if="index < 4" style="margin-bottom:20upx;"
								 v-for="(item,index) in Detail_info.lotteryCodes">{{item}}</view>
					   </view>
					   <view class="Friends-title" style="float:left;margin:10upx 20upx 0 0;">抽签结果</view>
					   <view class="codeitem" style="font-size:28upx;" 
							 v-if="Detail_info.winNumbering!=''">{{Detail_info.winNumbering}}</view>
					   <view class="codeitem" style="font-size:28upx;color: rgba(34, 34, 34, 0.4);border-color:rgba(34, 34, 34, 0.4);"
							 v-if="Detail_info.winNumbering ==''">未中签</view>   
				 </view>
				<view class="head-whole">
				     <view class="head-item"  @click="gohead(0)">
						  <view class="head-text" :class="head_index == 0 ? 'colt':''">
						  		作品故事介绍				 
						  </view>
						  <view class="head-border" v-if="head_index == 0"></view>
				     </view>
				     <view class="head-item"  @click="gohead(1)"> 
						  <view class="head-text" :class="head_index == 1 ? 'colt':''">
						 		链上信息				 
						  </view>
						  <view class="head-border" v-if="head_index == 1"></view>
				     </view>
				</view>
		  </view>
		 <view style="padding-bottom: 180upx;" v-if="head_index == 0">
				<view class="introduce-img" v-for="item in Detail_info.descPics">
		 ...
		 <view style="padding-bottom: 180upx;" v-if="head_index == 1">
			 <view class="artwork-back">
				  <view class="author-item" style="padding-bottom: 22upx;">
						<view class="left-author" style="padding-bottom: 10upx;">Hash值：</view>
						<view class="right-author">
							 {{Detail_info.hash}}
							 <image src="../../../static/images/fz.png" class="fuzi" @click="gocopy(Detail_info.hash)"></image>
						</view>
				  </view>
			 </view>	 
	 	 </view>	 
		  <view class="fixed-bottom">
			    <view class="left-fixed">
					  <view class="fixed-Price">
						    ￥{{Detail_info.salePrice}}
					  </view>
				</view>
				<view class="fixed-button" v-if="Detail_info.buyStatus == 0" 
				      @click="choicePayType('bottom')">购买</view>
				<!-- 没有优先购时间 -->
				<view class="fixed-button backs font" v-if="Detail_info.buyStatus == 1 && Detail_info.firstTime == 0">
				     {{Detail_info.startTime}}</view>
				<!-- 有优先购时间 -->
				<view class="fixed-button backs advance" v-if="Detail_info.buyStatus == 1 && Detail_info.firstTime > 0">
				      <view class="advance-item">{{Detail_info.startTime}}开售</view>
				 	  <view class="advance-text">可提前{{Detail_info.firstTime}}分钟购买</view>
			    </view>
				<!--2即将开售-->
				<view class="fixed-button backs font" v-if="Detail_info.buyStatus == 2">{{Detail_info.startTime}}</view>
				<!-- 优先购 -->
				<view class="fixed-button advance" v-if="Detail_info.buyStatus == 3" @click="choicePayType('bottom')">
				 		<view class="advance-item">提前购买</view>
						<view class="advance-text">{{Detail_info.startTime}}</view>
				</view>
				<view class="fixed-button backs" v-if="Detail_info.buyStatus == 4">已结束</view>
				<view class="fixed-button backs" v-if="Detail_info.buyStatus == 5">已售罄</view>
				<view class="fixed-button backs advance" v-if="Detail_info.buyStatus == 6">
					    <view class="advance-item">预约抽签</view>
				 		<view class="advance-text">{{Detail_info.drawLotsStartTime}} 开始</view>		
				</view>
				<view class="fixed-button advance"  @click="golottery('bottom')" 
				      v-if="Detail_info.buyStatus == 7 && Detail_info.isParticipate == 2">
					    <view class="advance-item">预约抽签</view>
				 		<view class="advance-text">{{Detail_info.drawLotsEndTime}}结束</view>
				</view>
				<view class="fixed-button advance"  style="width:280upx;"  @click="puttoggle('center')"
				      v-if="Detail_info.buyStatus == 7 && Detail_info.isParticipate == 1 && Detail_info.useOrNotWin == 2">
					    <view class="advance-item">邀请好友预约</view>
				 		<view class="advance-text">{{Detail_info.drawLotsEndTime}}结束</view>				
				</view>
				<view class="zhonq-button"  @click="golottery('bottom')"
				      v-if="Detail_info.buyStatus == 7 && Detail_info.isParticipate == 1 && Detail_info.userRightsId > 0 && Detail_info.useOrNotWin == 2">使用中签卡</view>
				<view class="fixed-button advance"
				       v-if="Detail_info.buyStatus == 7 && Detail_info.useOrNotWin == 1">
						<view class="advance-item">已使用中签卡</view>
						<view class="advance-text" style="padding:0;">{{Detail_info.publicBuyTime}} 前付款</view>	
				 </view>
				<view class="fixed-button advance"
				      v-if="Detail_info.buyStatus == 8">
					   <view class="advance-item">抽签中</view>
					   <view class="advance-text">{{Detail_info.priorityBuyTime}}公布结果</view>
				</view>
				<view class="fixed-button advance"  @click="choicePayType('bottom')"
				      v-if="Detail_info.buyStatus == 9">
					   	<view class="advance-item">已中签去购买</view>
				 		<view class="advance-text">{{Detail_info.publicBuyTime}}前付款</view>
				</view>
				<view class="fixed-button backs"  v-if="Detail_info.buyStatus == 10">中签已购买</view>
				<view class="fixed-button backs" v-if="Detail_info.buyStatus ==11">未中签</view>
		  </view>
		  ...
		  <uni-popup ref="popup"  @change="change">
				...
				<!--	  <view class="PayType-item">
						   ...
						   <view class="PayType-dunt"  @click="gochoice(1)"
		 			  ...
		 			  <view class="PayType-item">
						   ...
						   <view class="PayType-text">支付宝支付</view>
						   <view class="PayType-dunt"  @click="gochoice(2)"
								:class="payType == 2 ? 'none':''">
								 <image src="../../../static/images/xz2.png" v-if="payType == 2"></image>
						   </view>
					  </view> -->
				<!-- 	  <view class="PayType-item">
						   <view class="PayType-img">
								 <image src="../../../static/images/sd.png"></image>
						   </view>
						   <view class="PayType-text">杉德快捷支付</view>
						   <view class="PayType-dunt"  @click="gochoice(3)"
								 :class="payType == 3 ? 'none':''">
								 <image src="../../../static/images/xz2.png" v-if="payType == 3"></image>
						   </view>
					   </view> --> 
					 ...
					 <view class="order-item" v-if="Detail_info.buyStatus == 3">
						   <view class="order-left">可用优先抢购卡</view>
						   <view class="order-right">X{{Detail_info.userRightsNum}}</view>
					 </view>
					 <view v-if="honey_index == 1">
							<view class="order-item">
								  <view class="order-left">使用蜜糖</view>
								  <view class="order-right">{{Detail_info.salePrice}}</view>
							</view>
		   ...
		   <uni-popup ref="popup_gont"  @change="change">
				   <view class="popup-content" :class="{ 'popup-height': gont_type === 'center'}">	
						...
						<view class="popup-button" @click="gocopy(invitePageUrl)">复制链接发给好友</view>
				   </view> 
		    </uni-popup>
		   <uni-popup ref="popup_lottery"  @change="change">
				 <view class="PayType-content" style="min-height:440upx;"
						 :class="{ 'popup-height': lotterytype === 'bottom'}">
						   <view class="lottery-text" style="margin-top:40upx;">你有{{Detail_info.userRightsNumber}}张中签卡</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			gochoujqian(){
			  this.userRightsId = this.Detail_info.userRightsId
			  this.godrawLots()
			},
			godrawLots(){
			   let that = this
			   ...
			   uni.request({
					url: getApp().globalData.http_url + 'NfrArtwork/drawLots', 
				 method:'GET',
				 header:{
				 ...
				 data:{
					...
					id:that.art_id,
					userRightsId:that.userRightsId,
				 },
					success: (res) => {   			
			...
			goplus(){
			   var num = this.Detail_info.buyStatus == 3 ? this.Detail_info.userRightsNum : this.Detail_info.limitNum
			   if(this.purch == num){
				   return
			   }else{
				   this.purch++
			...
			golottery(type){
			  if(this.Detail_info.userRightsId > 0){
		   ...
		   gopurchase(){
			 ...
			 if(that.honey_index == 1){
			 	uni.request({
			 	      url: getApp().globalData.http_url + 'NfrArtwork/createOrder', 
			 	 	 method:'GET',
			 	 	 header:{
			 	 		 'content-type': 'text/json;charset=UTF-8',
			 	 		 'sessionId': that.sessionId
			 ...
			 if(that.payType == 4){
			    ...
			    uni.request({
			      ...
			      success: (res) => {   			
					...
					uni.hideLoading()
					if(res.data.code == 200){		
						if(that.Detail_info.salePrice == 0){
							uni.showToast({
								title: '购买成功！',
								icon: "success",
								duration: 1000
		   ...
		   getArtworkDetail(){
			   let that = this
			   uni.request({
			     url: getApp().globalData.http_url + 'NfrArtwork/detail', 
			   	 method:'GET',
			   	 header:{
			   		 'content-type': 'text/json;charset=UTF-8',
			   		 'sessionId': uni.getStorageSync('sessionId')
...
<style>
	...
	.artwork-back {
		width: 100%;
		padding: 20upx 34upx;
		background: rgba(48, 57, 105, 0.05);
		border-radius: 16upx;
	}
	.author-item {
		width: 100%;
